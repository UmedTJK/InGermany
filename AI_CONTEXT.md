...
## 📡 Архитектура загрузки данных (DataService)

В приложении реализована стратегия **offline-first** для работы со статьями, категориями и локациями.  

### Принципы
1. **Кэш в памяти**  
   - Повторные запросы обслуживаются напрямую из `DataService` без сети и без чтения файлов.  

2. **Локальный fallback**  
   - При первом запуске данные берутся из `Resources/*.json` (articles, categories, locations).  
   - Это гарантирует мгновенный вывод интерфейса без задержек.  

3. **Фоновое обновление из сети**  
   - Параллельно запускается запрос к GitHub Pages (через `NetworkService`).  
   - При успехе данные в кэше заменяются свежими.  

4. **Pull-to-refresh**  
   - При ручном обновлении (`refreshable`) кэш очищается.  
   - Далее снова идёт цепочка: локальный JSON → сеть.  

5. **Отладка**  
   - В `DataService` встроены `print`-логи, которые фиксируют источник:  
     - `📦 ... из памяти (cache)`  
     - `📂 ... из локального JSON`  
     - `🌐 ... обновлены из сети`  
     - `🗑️ Кэш очищен`
...
### UI-индикация
В `HomeView` отображается цветная полоска (и может быть подпись), показывающая источник данных:  
- 🟩 `network` — данные из сети  
- 🟦 `memory_cache` — данные из памяти  
- 🟧 `local` — данные из локального JSON  
- ⬜️ `unknown` — источник не определён

В Xcode логи попадают в Debug area (консоль). Чтобы их увидеть:

Запусти проект на симуляторе или на iPhone через ⌘R.

Внизу окна Xcode должна открыться панель отладки. Если её нет — включи:

меню View → Debug Area → Activate Console

или нажми ⌘⇧Y.

Там будут все твои print(...).
Например:

📂 Articles из локального JSON
🌐 Articles обновлены из сети
📦 Categories из памяти (cache)
⚠️ Ошибка загрузки Locations из сети: The Internet connection appears to be offline.
---
